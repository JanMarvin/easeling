...
- An opaque raster cell no longer bleeds half a cell into a transparent
  ...
  transparent cell, so the bleed ate into it.
- `width` and `height` take a `units` argument: `"in"`, `"cm"`, `"mm"`
  or `"px"` at 96 per inch.
...
- [`easel_xml()`](../reference/easel_xml.md) renders plotting code
  straight to a DrawingML string with no file involved;
  `easel_dev(file=)` remains the file-based device.
- [`easel_size()`](../reference/easel_size.md) and
  `easel_dev(dims =, wb =)` size the device from real workbook cell
  geometry.
